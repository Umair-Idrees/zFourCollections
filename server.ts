...
app.get('/api/products', async (req, res) => {
  try {
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({ error: 'Database not connected. Please check your MONGO_URI and password.' });
    }
...
// Explicitly handle 404 for API routes to avoid HTML responses
app.use('/api/*', (req, res) => {
